...
import { Part2Question } from "../types/questionTypes";
import { demoPart2Questions } from "../data/demoData";
import "../admin.css";
const Part2QuestionForm: React.FC = () => {
  const [questions, setQuestions] =
    useState<Part2Question[]>(demoPart2Questions);
  const [currentQuestion, setCurrentQuestion] = useState<Part2Question>({
    question: "",
    choices: { A: "", B: "", C: "", D: "" },
    correctAnswer: "A",
    status: "draft",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newQuestion: Part2Question = {
      ...currentQuestion,
      id: `p2-${Math.floor(Math.random() * 1000)
        .toString()
        .padStart(3, "0")}`,
      createdAt: new Date(),
      updatedAt: new Date(),
      ...
      <CCol xs={12}>
        <CCard className="mb-4 shadow-sm">
          <CCardHeader className="bg-primary text-white">
            <strong>
              {mode === "create" ? "Tạo" : "Chỉnh sửa"} Câu hỏi Part 2
            </strong>
          </CCardHeader>
          <CCardBody>
            {alert.visible && (
              ...
              <CRow className="mb-3">
                <CCol md={6}>
                  ...
                  <CFormInput
                    ...
                    required
                    placeholder="e.g., Where is the marketing report?"
                  />
                </CCol>
                <CCol md={6}>
        ...
        <CCard className="mb-4 shadow-sm">
          <CCardHeader className="bg-primary text-white">
            <strong>Danh sách câu hỏi Part 2</strong>
          </CCardHeader>
          <CCardBody>
